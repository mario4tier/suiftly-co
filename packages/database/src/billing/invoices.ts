...
export interface InvoiceLineItem {
  ...
  creditMonth?: string;
  description?: string; // Optional: extra context appended to semantic description (e.g., "Pro → Enterprise")
}
...
export async function createAndChargeImmediately(
  ...
  if (params.lineItem) {
    await tx.insert(invoiceLineItems).values({
      ...
      description: params.lineItem.description || null,
    });
  }
...
/**
 * Delete an unpaid invoice completely
 *
 * Use this instead of voidInvoice when the invoice was never paid and
 * we want to prevent database bloat (e.g., subscribe without funds → cancel).
 *
 * This is safe ONLY for invoices that:
 * - Have status 'pending' or 'failed' (never paid)
 * - Have no invoice_payments records
 *
 * Deletes related records in correct order to respect FK constraints:
 * 1. billing_idempotency (if any)
 * 2. invoice_line_items (would cascade, but explicit is clearer)
 * 3. billing_records
 *
 * @param tx Transaction handle
 * @param invoiceId Invoice ID to delete
 */
export async function deleteUnpaidInvoice(
  tx: DatabaseOrTransaction,
  invoiceId: number
): Promise<void> {
  // Safety check: only delete unpaid invoices
  const [invoice] = await tx
    .select({ status: billingRecords.status })
    .from(billingRecords)
    .where(eq(billingRecords.id, invoiceId))
    .limit(1);
  ...
  if (invoice.status === 'paid') {
    throw new Error(`Cannot delete paid invoice ${invoiceId}`);
  }

  // Delete in FK order
  await tx
    .delete(billingIdempotency)
    .where(eq(billingIdempotency.billingRecordId, invoiceId));

  // invoice_line_items would cascade, but explicit delete is clearer
  await tx
    .delete(invoiceLineItems)
    .where(eq(invoiceLineItems.billingRecordId, invoiceId));
