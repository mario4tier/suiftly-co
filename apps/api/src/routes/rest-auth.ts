 ...
 *
 * Mounted at /i/auth/* (internal endpoints)
 */
...
export async function registerAuthRoutes(server: FastifyInstance) {
   ...
   */
  server.post('/i/auth/connect', async (request: FastifyRequest, reply: FastifyReply) => {
    // Validate input
    const result = walletConnectSchema.safeParse(request.body);
    if (!result.success) {
   ...
   */
  server.post('/i/auth/verify', async (request: FastifyRequest, reply: FastifyReply) => {
    // Validate input
    const result = verifySignatureSchema.safeParse(request.body);
    if (!result.success) {
      return reply.status(400).send({
        error: 'Validation error',
        details: result.error.errors,
    ...
    const isNewCustomer = customer.length === 0;
   ...
   */
  server.post('/i/auth/refresh', async (request: FastifyRequest, reply: FastifyReply) => {
   ...
   */
  server.post('/i/auth/logout', async (request: FastifyRequest, reply: FastifyReply) => {
    const refreshToken = request.cookies.refreshToken;
